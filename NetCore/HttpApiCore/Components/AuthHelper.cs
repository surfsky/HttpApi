...
namespace App.HttpApi
{
    ...
    internal class AuthHelper
    {
        ...
        public static IPrincipal CreateCookieTicket(string user, string[] roles, string domain, string cookieName, DateTime expiration)
        {
            ...
            //Asp.Current.User = new UserRolePrincipal(new FormsIdentity(ticket), roles);

            // cookie
            var ticketString = FormsAuthentication.Encrypt(ticket);
            Asp.Current.Response.Cookies.Append(cookieName, ticketString, new CookieOptions { Expires=expiration, Domain=domain });
            Asp.Current.User = new UserRolePrincipal(new FormsIdentity(ticket), roles);

            return Asp.Current.User;
        }
        ...
        public static IPrincipal LoadPrincipalFromCookie()
        {
            ...
            try
            {
                ...
                Asp.Current.User = new UserRolePrincipal(new FormsIdentity(authTicket), roles);
            }
            catch (Exception ex)
            {
                Asp.Current.User = null;
            }
            return Asp.Current.User;
        }
        ...
        public static void Logout()
        {
            ...
            if (Asp.Current.Session != null)
                Asp.Current.Session.Clear();    //.Abandon();
        }

        private static void ClearAuthCookie()
        {
            string cookieName = FormsAuthentication.FormsCookieName;
            var cookie = Asp.Current.Request.Cookies[cookieName];
            if (cookie != null)
        ...
        public static List<string> GetRoles()
        {
            ...
            if (IsLogin())
            {
                FormsAuthenticationTicket ticket = ((FormsIdentity)Http.Current.User.Identity).Ticket;
