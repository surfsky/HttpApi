...
namespace App.HttpApi
{
    ...
    public partial class HttpApiHelper
    {
        ...
        public static void ProcessRequest(HttpContext context, object handler)
        {
            ...
            var url = context.Request.Path.Value.ToLower(); //.Url.AbsolutePath.ToLower();
            System.Diagnostics.Trace.WriteLine($"IP={ip}, URL={url}");
            if (IPFilter.IsBanned(ip))
            {
                //Asp.Current.Request.Abort();
                //Asp.Current.Request.Abort();
                return;
            }
            ...
            try
            {
                ...
                else
                {
                    ...
                    if (args.ContainsKey("_refresh"))
                    {
                        ...
                        if (refresh)
                             CacheHelper.GetCache<object>(cacheKey, expireDt, () => { return method.Invoke(handler, parameters); });
                    }
                    result = CacheHelper.GetCache<object>(cacheKey, expireDt, () => { return method.Invoke(handler, parameters); });
                }
        ...
        {
            //var path = Asp.Current.Request.FilePath;
            //var u = Asp.Current.Request.GetAbsoluteUri(); //.Url.AbsolutePath;
            //var url = u.ToString().ToLower();
            var uri = Asp.Current.Request.Path.ToString().ToLower(); //.GetAbsoluteUri().ToLower();  // http://..../a/b/c
            var path = new Url(uri).AbsolutePath;
        ...
        private static void CheckMethod(HttpContext context, MethodInfo method, HttpApiAttribute attr, Dictionary<string,object> inputs)
        {
            ...
            // 校验方法可用性
            //AuthHelper.LoadPrincipalFromCookie();  // 获取身份验票（由系统自己摆平了，注意将HttpApi中间件放在Authenticate后面即可）
        ...
        static void CheckMethodEnable(HttpContext context, MethodInfo method, HttpApiAttribute attr)
        {
            ...
            if (!attr.AuthVerbs.IsEmpty())
            {
                ...
                if (verbs.Count == 0)
                    return;
                if (!verbs.Contains(context.Request.Method.ToLower()))
        ...
        public static void WriteResult(
            ...
            int cacheSeconds = 0, ResponseCacheLocation cacheLocation = ResponseCacheLocation.None, 
        ...
        static ResponseType ParseDataType(Type t)
        {
            ...
            if (IsType(t, typeof(Image)))                 return ResponseType.Image;
        ...
        {
            var uri = Asp.Current.Request;
            var rootUrl = string.Format("{0}://{1}/HttpApi/{2}", uri.Scheme, uri.Host, GetTypeTrimName(type));
            return rootUrl;
        }
        ...
        private static string GetTypeTrimName(Type type)
        {
            ...
            if (!typePrefix.IsEmpty() && typeName.StartsWith(typePrefix, StringComparison.OrdinalIgnoreCase))
